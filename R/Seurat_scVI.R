...
run_scVI = function(
  ...
  message('Import scVI ...')
   ...
   message('Run scVI ...')
     vae = scvi$models$VAE(
               as.integer(nrow(cnts)),
               as.integer(n_batches),
               as.integer(20))
   ...
   message('Return scVI ...')
     ...
     rownames(latent$latent) = colnames(seu@raw.data)
     latent$latent = as.data.frame(latent$latent)
     colnames(latent$latent) = paste('vae', colnames(latent$latent), sep='_')

     rownames(latent$imputed) = rownames(seu@raw.data)
     colnames(latent$imputed) = colnames(seu@raw.data)
     return(latent)
}
...
Run_Seurat_scVI = cacheFile(path_RDS) %@% function(
  ...
  seu = SetAssayData(seu, slot='counts', assay='VAE', new.data=Matrix(imp))
  DefaultAssay(seu) = 'VAE'
  seu = ScaleData(object = seu, vars.to.regress='nGene')
  seu = FindVariableFeatures(seu, do.plot=FALSE)

  message('PCA ...')
  seu = RunPCA(seu, do.print=FALSE, dims.use=10)
  ...
  seu@meta.data = seu@meta.data %>%
    tidyr::separate(CellID, into=c('organism', 'cell', 'status', 'time', 'batch','plate', 'barcode'), sep='_', remove = FALSE) %>%
    magrittr::set_rownames(.$CellID)

  message('TSNE ...')
    seu = RunTSNE(seu,do.print=FALSE, dims.use = 1:dims.use, reduction.use='VAE')

  message('UMAP ...')
    seu = RunUMAP(seu, do.print=FALSE, dims.use = 1:dims.use, reduction.use='VAE', do.plot=FALSE)

  seu@meta.data = seu@meta.data %>%
    magrittr::set_rownames(.$CellID)   %>%
    cbind(seu@dr$tsne@cell.embeddings) %>%
    cbind(seu@dr$umap@cell.embeddings)

  seu = FindNeighbors(object  = seu, dims = 1:dims.use, verbose = FALSE)
  res = seq(0.2, 2, 0.2)
  seu = FindClusters(
    seu,
    resolution = res,
